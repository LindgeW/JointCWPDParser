...
class TransformerXL(nn.Module):
    ...
    def forward(self, h, seq_mask=None):
        ...
        # if self.training:
        #     attn_mask = torch.triu(all_ones, M + 1)[:, :, None]
        # else:
        #     # attn_mask = (torch.triu(all_ones, M+1) + torch.tril(all_ones, 0))[:, :, None]
        #     mask_shift_len = self.mem_len - M
        #     attn_mask = (torch.triu(all_ones, M + 1) + torch.tril(all_ones, -mask_shift_len))[:, :, None]
