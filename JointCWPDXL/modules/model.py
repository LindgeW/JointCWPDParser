...
class ParserModel(nn.Module):
    def __init__(self, args, embed_weights=None):
        ...
        self.char_embedding = nn.Embedding(num_embeddings=args.vocab_size,
                                           embedding_dim=args.char_embed_dim,
                                           padding_idx=0)

        self.pretrained_char_embedding = nn.Embedding.from_pretrained(torch.from_numpy(embed_weights), freeze=True)

        d_model = 2 * args.char_embed_dim

        # self.conv_encoder = ConvEncoder(in_features=d_model, out_features=d_model)

        # self.gru = GRU(input_size=d_model, hidden_size=d_model // 2,
        ...
        # self.att_layer = GateDotProductAttention(k_dim=d_model)
        # self.att_layer = DotProductAttention(k_dim=d_model)
        self.att_layer = LocalDotProductAttention(k_dim=d_model)
    ...
    def forward(self, ngrams, extngrams, mask=None):
        ...
        bigram_embed = self.char_embedding(bigram)
        ext_unigram, ext_bigram = extngrams
        unigram_embed = unigram_embed + self.pretrained_char_embedding(ext_unigram)
        bigram_embed = bigram_embed + self.pretrained_char_embedding(ext_bigram)

        ngram_embed = torch.cat((unigram_embed, bigram_embed), dim=-1).contiguous()
