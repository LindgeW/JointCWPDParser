...
def greedy_decode(arc_matrix, mask=None):
    ...
    '''
    if mask is None:
        mask = torch.ones(arc_matrix.shape[:2], device=arc_matrix.device).byte()
    mat = arc_matrix + torch.diag(arc_matrix.new(arc_matrix.size(1)).fill_(-1e9))
    mat.masked_fill_(~mask.unsqueeze(1), -1e9)
    heads = mat.data.argmax(dim=2)
    heads *= mask.long()
    return heads
