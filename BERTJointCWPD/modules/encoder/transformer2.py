...
class SelfAttention(nn.Module):
    ...
    def forward(self, q, k, v, pad_mask=None):
        ...
        att_weights = torch.matmul(q, k.transpose(-1, -2)).div(math.sqrt(k.size(-1)))

        if pad_mask is not None:
            att_weights.masked_fill_(pad_mask, -1e9)  # float('-inf')
...
class TransformerEncoder(nn.Module):
    def __init__(self, nb_layers, nb_heads, d_model, d_ff):
